...
export function CompanySelector({ currentCompanyId }: CompanySelectorProps) {
  ...
  if (isLoading) {
    return (
      <Button variant="outline" disabled className="flex items-center gap-2 text-muted-foreground border-border/50 px-3 py-1.5 h-9">
  ...
  if (error || !currentCompany) {
    return (
      <Button variant="outline" disabled className="flex items-center gap-2 border-red-300/30 text-red-500 dark:text-red-400 px-3 py-1.5 h-9">
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="w-56 bg-card/95 backdrop-blur-sm border border-border/30 shadow-md dark:shadow-primary/20 dark:border-primary/20 rounded-lg p-1 animate-in fade-in-80 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2">
        ...
        {companies.length === 0 ? (
          <div className="px-3 py-2.5 text-sm text-muted-foreground italic rounded-md bg-muted/50 mx-1 my-1">No tienes compañías disponibles</div>
        ) : (
