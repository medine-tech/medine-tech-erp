...
export function UserProfile() {
  ...
  // Si hay información del usuario del contexto, usarla
  // Si no, obtenerla del servicio como respaldo
  const userName = userInfo?.name ?? authService.getUserName();
  const userEmail = userInfo?.email ?? "";

  // Efecto para refrescar la información del usuario cuando sea necesario
  useEffect(() => {
    if (isAuthenticated && !userInfo) {
      const fetchUserData = async () => {
        try {
          ...
          // No necesitamos actualizar el estado local, el contexto ya lo hará
        } catch (error) {
